...
CASE WHEN a.recommender = 0 THEN 0 \
     WHEN a.recommender BETWEEN 1 and 999999999 THEN a.recommender \
     ELSE 0 \
     END AS recommender, \
a.gender, \
...
df1.to_csv("df1_mem_all.csv", index=False )
df1.loc[df1.recommender!=0,:].to_csv("df1_mem.csv", index=False )
...
dfpcarev = dfpcarev.iloc[1:N,0:4]
dfpcarev.columns = ['mem_no','revPC0','revPC1','revPC2']
