...
packages <- c('sqldf','plyr','ggplot2','lattice','RODBC','reshape2','igraph','ggmap','stargazer','maps',
              'MASS','vcd','Matrix','huge','ff','ffbase','randomForest','bigmemory','biganalytics','biglm',
              'bigalgebra','lubridate','proxy','lmtest','lmtest','xts','TTR','memisc','lme4','devtools',
              'network','sna')
for(i in 1:length(packages)) {
  if(!require(packages[i])) install.packages(packages[i])
}

setwd("C:\\Users\\sdowning\\Google Drive\\PhD\\Dissertation\\3. network analysis\\data")
library(sqldf)
library(sdowninMisc)
#library(stargazer)
library(plyr)
...
con <- odbcConnect(dsn = "cconma", uid = "root", pwd = "")

# --------------------------------------------------------------
q <- '
SELECT  DISTINCT CONCAT(YEAR(order_date),"-",MONTH(order_date),"-",DAY(order_date)) as pd
FROM    cconma_order;
'
pd <- sqlQuery(con,q)
...
# list of days between orders 
tb2list <- dget("tb2list.Rdata")
# 
tb2 <- dget("tb2.Rdata")


s1 <- function(n=1,mu=1,theta=.3){return(rnegbin(n,mu,theta))}
s2 <- function(n=1,mu=3,theta=.7){return(rnegbin(n,mu,theta))}
vocab <- c('cconma','other')

docs <- list()
for (i in 1:100){
  docs[[i]] <- matrix(as.integer(c(0,1,s1(),s2())),ncol=2,byrow = T)
}

m1 <- lda::lda.collapsed.gibbs.sampler(
          docs, 2, vocab, alpha=.1, eta=.1,
          num.iterations = 25, burnin=100, compute.log.likelihood = T)

t(m1$topics)
round(t(m1$topics / sum(m1$topics)),4)

rowSums(t(m1$topics)) / sum(m1$topics)

t(m1$topic_sums)
t(m1$topic_sums / sum(m1$topic_sums))
...
SELECT  c.mem_no AS mem_no,
        ...
        Count(c.mem_no) as cnt
FROM    cconma_member AS a
JOIN    cconma_product_review AS c
